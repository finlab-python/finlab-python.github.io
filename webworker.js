...
async function loadPyodideAndPackages() {
  ...
  try {
    ...
    await self.pyodide.loadPackage(['micropip', 'numpy', 'pandas', 'jinja2', 'requests']);
    ...
    await self.pyodide.loadPackage(['/pyodide/TA_Lib-0.4.26-cp311-cp311-emscripten_3_1_33_wasm32.whl']);
    self.postMessage({ content: '安裝 talib (計算技術指標)', type: 'text', id: currentId, finish: false })
    await self.pyodide.loadPackage(['pyodide/finlab-1.1.1-cp311-cp311-emscripten_3_1_46_wasm32.whl']);
...
onmessage = async function (e) {
  ...
  try {
    ...
    if (python.indexOf('plot') !== -1 && !matplotlibInstalled) {
      await self.pyodide.loadPackage(['Pillow', 'cycler', 'fonttools', 'kiwisolver',  'pyparsing', 'python-dateutil', 'pytz']);
      await self.pyodide.loadPackage(['/pyodide/matplotlib-3.5.2-cp311-cp311-emscripten_3_1_46_wasm32.whl']);
