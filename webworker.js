...
async function loadPyodideAndPackages() {
  ...
  try {
    ...
    await self.pyodide.loadPackage(['micropip', 'pandas', 'jinja2', 'requests', 'hashlib', 'setuptools', 'tzdata']);
    self.postMessage({ content: '安裝 pandas     (數據運算使用)', type: 'text', id: currentId, finish: false })

    // Stage 2: scipy + pyarrow + finlab + ta_lib in parallel
    await Promise.all([
      self.pyodide.loadPackage(['scipy']),
      self.pyodide.loadPackage(['pyarrow']),
      self.pyodide.loadPackage(['/pyodide/finlab-2.1.1-cp313-cp313-pyemscripten_2025_0_wasm32.whl']),
    ...
    // Custom wheels do not automatically install their pure-Python dependencies.
    await self.pyodide.runPythonAsync('import micropip\nawait micropip.install("croniter==1.3.8")');
